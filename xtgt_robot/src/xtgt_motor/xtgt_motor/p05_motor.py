...
class MotorController(Node):
    def __init__(self):
        ...
        self.data = [0xFF, 1, 1, 0, 1, 0, 3, 0, 0, 0, 0]      # [_, enable, left_dir, left_pwm, right_dir, right_pwm, mnq, rst, ban1, ban2, ban3] 10
        ...
        self.create_subscription(Int32, 'rst', self.rst_callback, 10)
        self.create_subscription(Int32MultiArray, 'ban_channel', self.ban_channel_callback, 10)
        self.odom_pub = self.create_publisher(Odometry, 'odom', 10)
        self.shot_log_pub = self.create_publisher(Int32, 'shot_signal', 10)
    ...
    def read_encoder(self):
        if self.ser.in_waiting > 0:
            try:
                ...
                if mnq != '0' and self.log_ok:
                    msg = Int32()
                    msg.data = int(mnq)
                    self.shot_log_pub.publish(msg)
                    self.log_ok = False
                elif mnq == '0':
                    self.log_ok = True
            except ValueError:
    ...
    def tx_pico(self, left_pwm, right_pwm):
        ...
        if self.stop == True:
            self.ser.write(bytes([0xFF, 0, 0, 0, 0, 0, self.mnq, self.rst, 0, 0, 0]))
        else:
            if left_pwm < 0:
                data[2] = 1
                data[3] = left_pwm * -1
            ...
            data[6] = self.mnq
            data[7] = self.rst
            data[8] = self.ban_list[0]
            data[9] = self.ban_list[1]
            data[10] = self.ban_list[2]
...
def main():
    ...
    try:
        ...
        try:
            mc.ser.write(bytes([0xFF, 0, 0, 0, 0, 0, 3, 0, 0, 0, 0]))
        except Exception:
            pass
    finally:
        mc.destroy_node()
